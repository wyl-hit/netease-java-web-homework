...
public class ProductDaoImplement implements ProductDao {
    ...
    public boolean updateProduct(Product product) {
        if(product.getSales() > 0){
            String hql = "update Product set name=?,description=?,keyWord=?,price=?,sales=?,counts=?,type=? where id=?";
            Query query = sessionFactory.getCurrentSession().createQuery(hql);
            ...
            query.setParameter(4,product.getSales());
    ...
    public List<Product> getUnBuyProducts() {
        String hql = "from Product where sales=0";
        Query query = sessionFactory.getCurrentSession().createQuery(hql);
        return query.list();
    }
}
